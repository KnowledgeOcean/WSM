...
public class TSLDAModel{

	private double alpha,gama,lambda,tao;	//超参(dirichlet分布函数的参数)
	...
	private int M,K,V,U,T,S,H;	//分别表示文档集中文档的个数，主题个数，字典中的词汇个数
	
	private double[][][] ou;	//{hashtag-情感}对下的主题分布  维度 H*S*K
	...
	private int[][][] nhsk;	//每一项为：{hashtag-情感}对下的特定词汇出现的次数   维度 H*S*K
	private int[][] nhskSum;	//每一项为：{hashtag-情感}对下的所有主题出现的次数和
	...
	private int[] h;		//每一项为：特定文档下的hashtag
	private int[] x;		//每一项为：特定文档下的主题
...
//	private int saveTime=0;	//保存次数计数变量
	...
	public TSLDAModel(Documents documents)
	{
	...
	private void allocateMemoryForVariables()
	{
		...
		H=ModelVariables.g_hashtagsSet.size();
		S=3; //情感类型
...
//		integratePrior.print();
	...
	public void initialiseModel()
	{
		...
		for (int m = 0; m < M; m++) {
			...
			if(document.hasHashtag){
				if(Math.random() < tao){
					x[m] = 1;
					int index = (int) (Math.random()*(document.getOwnHashtagList().size()));
					h[m] = document.getOwnHashtagList().get(index);
				}
				else{
					x[m] = 0;
					int index = (int) (Math.random()*(document.getPotentialHashtagList().size()));
					h[m] = document.getPotentialHashtagList().get(index);
				}
				nhsk[h[m]][sm][zm]++;
				nhskSum[h[m]][sm]++;
			}
	...
	private void sampleHashtagForDoc(int m)
	{
		Document document=documents.docs.get(m);
		int oldTopic=z[m];
		int Nm=document.docWords.length;	//第m篇文档的词数（长度）		
		...
		int oldHashtag = h[m];
		nhsk[oldHashtag][oldSentiment][oldTopic]--;
		nhskSum[oldHashtag][oldSentiment]--;
		
		ArrayList<String> sampleIndexs = new ArrayList<>();
		ArrayList<Double> probabilities = new ArrayList<>();
		double proTotal = 0.0;
		// x = 0
		double[][][] p=new double[document.getPotentialHashtagList().size()][S][K];
		int index = 0;
		for (Integer hashtagIndex : document.getPotentialHashtagList()) {
//			System.out.println(document.getPotentialHashtagList().toString());	
			for (int s = 0; s < S; s++) {
		  	    for (int k = 0; k < K; k++) {
		  	    	double pro=calCondiTionalPro(document,s,k);  
	  	    		p[index][s][k] = (1-tao)*(1.0 / document.getPotentialHashtagList().size())*
	  	    				(nms[m][s]+lambda)/(nmsSum[m]+S*lambda)*
	 						(nhsk[hashtagIndex][s][k]+alpha)/(nhskSum[hashtagIndex][s]+K*alpha)*
	 						pro;
	  	    		sampleIndexs.add("0-"+hashtagIndex+"-"+s+"-"+k);
	  	    		probabilities.add(p[index][s][k]);
	  	    		proTotal += p[index][s][k];
		...
		// x = 1
		p=new double[document.getOwnHashtagList().size()][S][K];
		index = 0;
		for (Integer hashtagIndex : document.getOwnHashtagList()) {
			for (int s = 0; s < S; s++) {
		  	    for (int k = 0; k < K; k++) {
		  	    	double pro=calCondiTionalPro(document,s,k);  
	  	    		p[index][s][k] = tao*(1.0 / document.getOwnHashtagList().size())*
	  	    				(nms[m][s]+lambda)/(nmsSum[m]+S*lambda)*
	 						(nhsk[hashtagIndex][s][k]+alpha)/(nhskSum[hashtagIndex][s]+K*alpha)*
	 						pro;
	  	    		sampleIndexs.add("1-"+hashtagIndex+"-"+s+"-"+k);
	  	    		probabilities.add(p[index][s][k]);
	  	    		proTotal += p[index][s][k];
		...
		double u = Math.random()* proTotal;
		int newSentiment = 0,newTopic = 0,newHashtag=0;
		
		double currentPro = 0.0;
		for (int i = 0; i < probabilities.size(); i++) {
			currentPro += probabilities.get(i);
			if(currentPro > u){
				String indexArray[] = sampleIndexs.get(i).split("-");
				newHashtag = Integer.valueOf(indexArray[1]);
				newSentiment=Integer.valueOf(indexArray[2]);
		    	newTopic=Integer.valueOf(indexArray[3]);
		    	break;
			}
		}
...
//			  	    	double pro=calCondiTionalPro(document,s,k);  
//			  	    	if(x==0){
//			  	    		p[x][s][k] = (ny[x]+gama)/(ny[0]+ny[1]+2*gama)*
//			 						(nms[m][s]+lambda)/(nmsSum[m]+S*lambda)*
...
//		y[m]=newY;
		nms[m][newSentiment]++;
		nmsSum[m]++;
		nhsk[newHashtag][newSentiment][newTopic]++;
		nhskSum[newHashtag][newSentiment]++;
	...
	private void calLatentVariables(boolean isFinalIteration)
	{
		...
		// 计算{hashtag-情感}对下的主题分布
		for (int h = 0; h < H; h++) {
			for (int s = 0; s < S; s++) {
				for (int k = 0; k < K; k++) {
					ou[h][s][k] += (nhsk[h][s][k]+alpha)/(nhskSum[h][s]+H*alpha);
					if(isFinalIteration)
						ou[h][s][k] = ou[h][s][k] / ((iterations-burn_in) / saveStep + 1);//saveTime;
				}
			}
		}
...
//		FilesUtil.saveCount(nms);
		FilesUtil.saveDistributions(delta, theta, ou);
